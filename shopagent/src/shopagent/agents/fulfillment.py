...
class FulfillmentAgent(Agent):
    ...
    def system_prompt(self) -> str:
        return (
            "You are the fulfillment agent for a Shopify dropshipping store that "
            "fulfills through CJ Dropshipping.\n"
            "Workflow each run:\n"
            "1. Call sync_shopify_orders to pull unfulfilled store orders into the "
            "local pipeline.\n"
            "2. For each order with status 'new': map its line items to CJ variants "
            "with map_line_items_to_cj. If every item maps, propose cj.create_order "
            "via propose_action with order_ref set to the Shopify order name, "
            "cj_items, the order's shipping address, and logistic_name "
            ...
            "item cannot be mapped, set status 'attention' with a note explaining what "
            "is unmappable instead.\n"
            "3. For each order with status 'cj_placed': call check_cj_order. When a "
            "tracking number first appears, do two things: (a) record it and set "
            "status 'shipped' with update_order_status, and (b) propose "
            "shopify.fulfill_order via propose_action with the order's "
            "shopify_order_id, the tracking number, and notify_customer true "
            "(ref_table='orders', ref_id=local order id) — on approval this marks "
            "the order fulfilled in Shopify and emails the customer their "
            "tracking. Propose it only once per order: skip orders already "
            ...
            "status is not 'new'. Finish with a summary: orders synced, proposals "
            "made, tracking updates, and anything needing attention."
        )
    ...
    def _sync_orders(self, inp: dict) -> list[dict]:
        for order in self.shopify.list_open_orders():
